...
- Options page (`destination-choices/index.html`): passphrase gate → hero with status tally + compare-table toggle → viewer (selected option: photos with captions/credits, A/F/S gauges, why-it's-listed, day/night itinerary, requirements, flags, timing) → sticky bottom strip of all 12 options (tap to open, ◀ ▶ buttons and arrow keys step through, URL hash deep-links, e.g. `#moab`).
- Data: 12 options in `TRIPS` (1 anchor, 5 contenders, 3 developing, 3 backups). Each trip: `id, name, region, status, cost:[lo,hi], scores:{A,F,S}, flight, wow, second, gates[], flags[], fit, days:[{d, day, night}], photos:[{src, thumb, alt, caption, credit}]`.
