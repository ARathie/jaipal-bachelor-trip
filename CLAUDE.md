...
- Data: 20 entries in `TRIPS` — the narrowed board (6 CONTENDER + 2 MONITOR) followed by the archive: 3 villa-era CUTs (Casa de Campo, Bocas, Costa Rica — `status:"CUT", archived:true`, cut reason as the first flag) and the 9 ARCHIVED adventure-era options. Everything `archived:true` is hidden from strip/compare unless the Archive toggle is on. **Archived/cut entries are permanent** (owner decision) — same rule as constraint 2. Scores are keyed per rubric and gauges/compare render labels from the keys: contenders `scores:{ST,NL,AC,NV,VQ}` (storm-risk scored as safety / nightlife / activities / novelty / villa quality, higher = better, from the shortlist scorecard); monitors and villa-era cuts keep `{C,F,W}`; adventure archive keeps `{A,F,S}`. Board entries also carry `mods:[{k,v,n}]` (activity tag bubbles, v: 0 none · 1 partial/caveat · 2 available · 3 strong, n = short note) and contenders add `pros[]`, `cons[]`, `villa`. Common fields: `id, name, region, status, cost:[lo,hi], flight, wow, second, gates[], flags[], fit, days:[{d, day, night}], photos:[{src, thumb, alt, caption, credit}]`.
